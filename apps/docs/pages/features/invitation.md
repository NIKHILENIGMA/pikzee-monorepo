...
- **Flow:**
  ...
  4. **Business Logic & Database Query:**
     - **Business Logic:**
       ...
       - Check if there is already a `PENDING` invite for this email in this workspace. If yes, overwrite the token to invalidate the old link. If no, insert a new record.
     ...
     - **Database Query:**
       ...
       - **Query 3:** Either `UPDATE` the existing pending invite (generating a new `nanoid` and resetting `expires_at` to now + 7 days) or `INSERT` a brand new `workspace_invitations` record.
...
- **Flow:**
  ...
  4. **Business Logic & Database Query:**
     ...
     - **Database Query:** `SELECT` from `workspace_invitations` where `workspace_id = :workspaceId` AND `status = 'PENDING'` AND `expires_at > now()`.
...
- **Error Status Codes:**
  ...
  - `409 Conflict` (Invite is already `ACCEPTED`)
  - `410 Gone` (Invite is `EXPIRED` or `REVOKED`)
- **Flow:**
  ...
  4. **Business Logic & Database Query:**
     - **Business Logic:**
       ...
       - Check the `status`. If `REVOKED` or `ACCEPTED`, return the corresponding error.
       - Check the `expires_at` timestamp. If it has passed, dynamically update the status to `EXPIRED` and return `410 Gone`.
     - **Database Query:**
       - `SELECT` from `workspace_invitations` filtering by `token`.
       - If expired, execute an `UPDATE` to set `status = 'EXPIRED'`.
...
- **Flow:**
  ...
  4. **Business Logic & Database Query:**
     - **Business Logic:**
       - Validate the token's existence, status, and expiry (same checks as the GET endpoint).
     ...
     - **Database Query (Atomic Transaction):**
       ...
       - **Query 3:** `UPDATE` the `workspace_invitations` table, setting `status = 'ACCEPTED'`, `accepted_at = now()`, and `accepted_by = currentUser.id`.
